...
[![Author: Kush Patel](https://img.shields.io/badge/author-Kush%20Patel-lightgrey)](docs/INTERVIEW_FAQ.md)
...
| GitHub | *this repository* | [`bdsp-core/Neurotech-EEG-Wrangling`](https://github.com/bdsp-core/Neurotech-EEG-Wrangling) |
| Contains EEG data? | **No** (code + lightweight metrics only) | N/A (points at BDSP download) |

**Dataset (cite this):** Morgan, K., Pickering, C., Goodwin, M., Wu, H., Ghanta, M., Gupta, A., Goldenholz, D., & Westover, M. B. (2026). *The Neurotech EEG Dataset* (version 1.0). Brain Data Science Platform. [https://doi.org/10.60508/v99k-ek82](https://doi.org/10.60508/v99k-ek82)
...
- End-to-end engineering: BIDS paths → Xltek weak labels → **patient-level splits (0 leakage)** → filter/resample → **~244 GB** window cache  
- Modern detector: **EEG Conformer** + focal loss + TTA + temporal smoothing  
- Clinically flavored metrics: **PR-AUC, PRG-AUC, precision@recall, event FA/24h**  
- Honest null: preictal **forecasting ≈ chance** (failure analysis below)  
...
**One-liner for credibility:** the Conformer + smoothing stack beats bandpower logistic regression by **+0.38 PR-AUC** (0.082 → 0.460) and beats a compact EEGNet CNN by **+0.29 PR-AUC** (0.174 → 0.460) on the same held-out patients.
...
## Why forecasting failed (short failure analysis)
...
Likely drivers:
1. **Scalp EEG preictal signal is weak / inconsistent** across patients at multi-minute horizons  
...
4. Patient-independent setting is much harder than patient-specific forecasting literature  
...
| Resource | Use |
|---|---|
| Neurotech EEG Dataset (BDSP) — DOI [10.60508/v99k-ek82](https://doi.org/10.60508/v99k-ek82) | Data source |
...
**Overall (applied ML portfolio): ~9/10 graduate / industry-prep.**  
Strong data engineering + evaluation; **not** a novel NeurIPS method paper (novelty ~6.5/10).

| Lens | Verdict |
|---|---|
| Entry DS / ML engineer hiring | **Excellent** — quantifiable scale, leakage hygiene, clinical metrics, honest failures |
